...
def sam_loss_masks(masks, gt_masks, gt_ids, num_points, oversample_ratio, importance_sample_ratio):
    """Compute the losses related to the masks: the focal loss and the dice loss.
    targets dicts must contain the key "masks" containing a tensor of dim [nb_target_boxes, h, w]
    """
    ...
    num_masks = torch.as_tensor(
        [num_masks], dtype=torch.float, device=masks.device
    )
    if is_dist_avail_and_initialized():
        torch.distributed.all_reduce(num_masks)
    num_masks = torch.clamp(num_masks / get_world_size(), min=1).item()

    src_masks = masks  # (n, 1, h, w)
    target_masks = gt_masks[gt_ids].to(src_masks)  # (n, 1, h, w)
